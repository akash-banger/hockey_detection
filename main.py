...
def main():
    ...
    tracker = Tracker(puck_model_path, is_puck_detection=True)
    tracks = tracker.get_object_tracks(frames, read_from_stub=True, stub_path='stubs/stub_tracks.pkl')
    team_assigner = TeamAssigner()
    
    if os.path.exists('stubs/tracks_with_teams.pkl'):
        with open('stubs/tracks_with_teams.pkl', 'rb') as f:
        ...
        with open('stubs/tracks_with_teams.pkl', 'wb') as f:
            pickle.dump(tracks, f)
    
    if os.path.exists('stubs/tracks_with_teams_and_jerseys.pkl'):
        with open('stubs/tracks_with_teams_and_jerseys.pkl', 'rb') as f:
            tracks = pickle.load(f)
    else:
        for frame_num, player_track in enumerate(tracks['players']):
            for player_id, track in player_track.items():
        ...
        with open('stubs/tracks_with_teams_and_jerseys.pkl', 'wb') as f:
            pickle.dump(tracks, f)
